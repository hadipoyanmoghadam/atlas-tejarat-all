...
public class FollowupSafeBox extends HostHandlerBase {
    private static Log log = LogFactory.getLog(FollowupSafeBox.class);

    public HostResultSet _post(Object o, Map map, HostInterface hostInterface) throws CMFault {
        if (log.isInfoEnabled()) log.info("Inside FollowupSafeBox:_post()");
        ...
        if (SrcHostIdStr.equals(Constants.HOST_ID_FARAGIR)) {
            ...
            inputParams.put(Constants.ACC_NO, amxMessage.getSrcAccount().trim());

        } else if (DestHostIdStr.equals(Constants.HOST_ID_FARAGIR)) {
            // Faragir is destination account
            inputParams.put(Constants.ACC_NO, amxMessage.getDestAccount().trim());
        }

        String origMessageData =amxMessage.getOrigMessageData();
        ...
        inputParams.put(Constants.BRANCH_CODE, amxMessage.getBranchCode().trim());
