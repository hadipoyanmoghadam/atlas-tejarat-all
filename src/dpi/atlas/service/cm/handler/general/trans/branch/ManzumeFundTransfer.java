 ...
 * Time: 03:01 PM
 */
public class ManzumeFundTransfer extends HostHandlerBase {
    ...
    public HostResultSet _post(Object o, Map map, HostInterface hostInterface) throws CMFault {
        if (log.isInfoEnabled()) log.info("Inside ManzumeFundTransfer:_post()");
        ...
        host_id=Constants.HOST_ID_SGB+Constants.HOST_ID_FARAGIR;
        ...
        String opCode=Constants.MANZUME_DEPOSIT_OP_CODE;
        ...
        inputParams.put(Constants.ID1, command.getParam(Fields.MN_ID).trim());
        inputParams.put(Constants.ID2, command.getParam(Fields.PAYID2).trim());
        ...
        try {
            hrs = ((HostInterfaceNAB) hostInterface).brMnzFundTransfer(inputParams);
            if (hrs.next()) {
    ...
    private HostResultSet getHostResultSet(HostResultSet original_res, String host_id, String branchCode, String docNo) throws HostException {
        ...
        rowindex++;
        md.addColumn(rowindex, "signDestBalance");
        rowindex++;
        md.addColumn(rowindex, "Destbalance");
        rowindex++;
        md.addColumn(rowindex, "signDestAvailableBalance");
        rowindex++;
        md.addColumn(rowindex, "DestavailableBalance");
        ...
        row.add(original_res.getString("signDestBalance"));
        row.add(original_res.getString("Destbalance"));
        row.add(original_res.getString("signDestAvailableBalance"));
        row.add(original_res.getString("DestavailableBalance"));
