 ...
 * Time: 13:25 PM
 */
public class BlockHandler implements Configurable {
    private static Log log = LogFactory.getLog(BlockHandler.class);
    ...
    public String handleCFS(Block block, RowOfBlock blockRow, String blockNo) throws Exception {
        ...
        try {
            ...
            try {
                ...
                log.error("Can not zeropad sessionId  = '" + sessionId + "' in BlockHandler.HandleCFS() : " + e.getMessage());
            }
            try {
                userId = ISOUtil.zeropad(blockRow.getUserId().trim(), 6);
            } catch (ISOException e) {
                log.error("Can not zeropad userId  = '" + userId + "' in BlockHandler.HandleCFS() : " + e.getMessage());
            }
            ...
            command.addHeaderParam(Fields.MESSAGE_TYPE, TJCommand.CMD_FINANCIAL_GROUP_BLOCK);
            command.addHeaderParam(Fields.SERVICE_TYPE, Constants.BLOCK_SERVICE);
            command.addHeaderParam(Fields.COMMAND, TJCommand.CMD_FINANCIAL_GROUP_BLOCK);
            ...
            command.addParam(Fields.BRANCH_CODE, userId);//user set as branchCode in cfs block service
            ...
            command.addParam(Fields.ACCOUNT_STATUS, Constants.BLOCK_ACCOUNT_STATUS);
            command.setCommandName(TJCommand.CMD_FINANCIAL_GROUP_BLOCK);
            command.addParam(Fields.SRC_ACCOUNT, blockRow.getAccountNo());
            command.addParam(Fields.DOCUMENT_DESCRIPTION, "" != blockRow.getBlockDesc() ? blockRow.getBlockDesc() : "blank");
            command.addParam(Fields.BLCK_DATE, block.getFileDate());
            command.addParam(Fields.BLCK_TIME, block.getFileTime());
            ...
            log.error("Error in BlockHandler.HandleCFS()!!BatchPk= NOT FOUND!!!!!! ");
    ...
    public String handleFaragir(Block block, RowOfBlock blockRow, String blockNo) throws CMFault {
        ...
        try {
            ...
            log.error("Can not zeropad sessionId  = '" + sessionId + "' in BlockHandler.HandelFaragir() : " + e.getMessage());
        }
        try {
            userId = ISOUtil.zeropad(blockRow.getUserId(), 6);
        } catch (ISOException e) {
            log.error("Can not zeropad userId  = '" + userId + "' in BlockHandler.HandelFaragir() : " + e.getMessage());
        }
        try {
            branchCode = ISOUtil.zeropad(blockRow.getBranchCode(), 5);
        } catch (ISOException e) {
            log.error("Can not zeropad branchCode  = '" + branchCode + "' in BlockHandler.HandelFaragir() : " + e.getMessage());
        }

        CMMessage msg = new CMMessage();
        msg.setAttribute(Fields.COMMAND, TJCommand.CMD_FINANCIAL_GROUP_BLOCK);
        ...
        msg.setAttribute(Fields.BLCK_DATE, block.getFileDate());
        msg.setAttribute(Fields.BLCK_TIME, block.getFileTime());
        msg.setAttribute(Fields.DESC, "" != blockRow.getBlockDesc() ?blockRow.getBlockDesc() : "blank");
        ...
        msg.setAttribute(Fields.ACCOUNT_STATUS, Constants.BLOCK_ACCOUNT_STATUS);
