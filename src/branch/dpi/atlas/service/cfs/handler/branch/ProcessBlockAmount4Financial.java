 ...
 */
public class ProcessBlockAmount4Financial extends CFSHandlerBase implements Configurable {
    private static Log log = LogFactory.getLog(ProcessBlockAmount4Financial.class);
    protected String accountField;
    protected String amountField;

    public ProcessBlockAmount4Financial() {
    ...
    public void doProcess(CMMessage msg, Map holder) throws CFSFault {
        ...
        try {
            ...
            log.error("Can not zeropad account number = '" + acc + "' in ProcessBlockAmount4Financial : " + e.getMessage());
        }
        try {
            long minBlance = 0;
            ...
            chnUser = Constants.CHN_USER_BLOCK;
            blockNo = msg.getAttributeAsString(Fields.BLOCK_ROW);
            if (blockNo == null || blockNo.equals(""))
                throw new Exception("Error in blockNo");
            branchId = msg.getAttributeAsString(Fields.USER_ID);

            CFSFacadeNew.doBlockAmountTransactional4Financial(acc, minBlance, blockedAmount, blockNo, brokerId, proverId, chnUser, branchId,
                    documentDescribtion, createDate, createTime, accountStatus, blockDate, blockTime);
