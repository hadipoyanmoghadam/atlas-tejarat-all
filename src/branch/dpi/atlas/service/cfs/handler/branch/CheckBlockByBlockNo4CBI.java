 ...
 */
public class CheckBlockByBlockNo4CBI extends CFSHandlerBase implements Configurable {
    private static Log log = LogFactory.getLog(CheckBlockByBlockNo4CBI.class);
    ...
    public void doProcess(CMMessage msg, Map holder) throws CFSFault {
        ...
        try {
            msg.setAttribute(Fields.BLOCK_STATUS, Constants.NOT_DUPLICATE_BLOCK_STATUS);
            blockNo = msg.getAttributeAsString(Fields.BLOCK_ROW);
            String[] isDuplicateBlckNo = CFSFacadeNew.findBlockNo4CBI(blockNo, Constants.BROKER_ID_SIMIN_CBI, Constants.PROVIDER_ID_SIMIN_CBI, acc);
            if (isDuplicateBlckNo[0] != null && isDuplicateBlckNo[1] != null && isDuplicateBlckNo.length == 2) {
                String blockAmount=isDuplicateBlckNo[0].trim();
                String blockStatus=isDuplicateBlckNo[1];
                if (blockStatus.equals(Constants.DUPLICATE_UNBLOCK_STATUS)) {
                    ...
                    msg.setAttribute(Fields.BLOCK_AMOUNT, blockAmount);
                    msg.setAttribute(Fields.BLOCK_STATUS, blockStatus);
                }
            }
    ...
    public CheckBlockByBlockNo4CBI() {
