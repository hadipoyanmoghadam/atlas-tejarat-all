...
public class DoTransactionFromBranch extends CFSHandlerBase implements Configurable {
...
//            tx.setHostCode(Constants.CFS_HOSTID+ msg.getAttributeAsString(Constants.DEST_HOST_ID));
        ...
        if (messageType.equalsIgnoreCase(TJCommand.CMD_BRANCH_DEPOSIT) || messageType.equalsIgnoreCase(TJCommand.CMD_CREDITS_DEPOSIT)||messageType.equalsIgnoreCase(TJCommand.CMD_MANZUME_DEPOSIT)){
        ...
        try {
            ...
            if (tx.getTxCode().equalsIgnoreCase(TJCommand.CMD_BRANCH_WITHDRAW_GIFTCARD))
                throw new CFSFault(CFSFault.FLT_DOCUMENT_NOT_FOUND, ActionCode.DOCUMENT_NOT_FOUND);
            else
                throw new CFSFault(CFSFault.FLT_BILL_PAYMENT_BEFORE_PAID, ActionCode.BILL_PAYMENT_BEFORE_PAID);
        } catch (Exception e) {
            log.error(e);
