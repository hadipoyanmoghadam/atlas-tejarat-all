...
public class DoTransaction4Cancellation extends CFSHandlerBase implements Configurable {
    ...
    public void doProcess(CMMessage msg, Map holder) throws CFSFault {
        try {
            ...
            AccountData accountData = (AccountData) holder.get(fromAccountField);
            long bal = accountData.getAccountBalance();

            if (bal <= 0) {
                log.debug("Insufficient funds for branch credit account " + accountData.getAccountNo());
                throw new ModelException(ActionCodeMsg.NOT_SUFFICIENT_FUNDS);
            }
