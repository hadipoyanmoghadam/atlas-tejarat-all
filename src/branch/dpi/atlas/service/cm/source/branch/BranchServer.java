 ...
 * Time: 10:44:30 AM
 */
public class BranchServer extends Observable
        implements Runnable, Observer {
    private static Log log = LogFactory.getLog(BranchServer.class);
    ...
    public BranchServer(int port, BranchServerChannel clientSide, ThreadPool pool) {
        super();
        this.port = port;
    ...
    public BranchServer(int port, BranchServerChannel clientSide, ThreadPoolExecutor pool) {
        super();
        this.port = port;
        this.clientSideChannel = clientSide;
        this.clientSideChannelClass = clientSide.getClass();
                    ...
                    try {
                        ...
                        for (; ; ) {
                            ...
                            if (messageCounter++ % 1000 == 0) {
                                ...
                                log.fatal("ThreadPool active count: " + getActiveCount());
                                log.fatal("ThreadPool idle count: " + getIdleCount());
                                log.fatal("ConnectionPool available count: " + ChannelFacadeNew.getDbConnectionPool().getAvailableConnectionPoole());
                                log.fatal("ConnectionPool active count: " + ChannelFacadeNew.getDbConnectionPool().getActiveConnectionPoole());
                                log.fatal("ConnectionPool max active count: " + ChannelFacadeNew.getDbConnectionPool().getMaxActiveConnections());
                            }
                            ...
                            if (getIdleCount() == 0 && getActiveCount() >= getMaxPoolSize()) {
                                ...
                                messageString = messageString.substring(0, 60) + ActionCode.TOO_MANY_RECORDS_REQUESTED;
