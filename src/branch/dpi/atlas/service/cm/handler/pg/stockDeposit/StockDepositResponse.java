 ...
 * Time: 10:55 AM
 */
public class StockDepositResponse extends CMHandlerBase {
    ...
    public void doProcess(CMMessage msg, Map holder) throws CMFault {
        ...
        try {
            ...
            JAXBContext context = JAXBContext.newInstance(StockDepositResp.class);
            Marshaller marshaller = context.createMarshaller();
            marshaller.setProperty(Marshaller.JAXB_ENCODING, "UTF-8");
            marshaller.setProperty(Marshaller.JAXB_FORMATTED_OUTPUT, Boolean.TRUE);
            StockDepositResp stockDepositResp = new StockDepositResp();
            stockDepositResp.setActioncode(actionCode);
            stockDepositResp.setDesc(actionMsg);
            stockDepositResp.setRrn(stockDepositReq.getRrn());
            stockDepositResp.setRespdatetime(DateUtil.getSystemDate() + DateUtil.getSystemTime());

            marshaller.marshal(stockDepositResp, responseTypeXml);

            msg.setAttribute(CMMessage.RESPONSE, responseTypeXml.toString());
        } catch (Exception e) {
            log.error("ERROR :::Inside StockDepositResponse.doProcess(): " + e.getMessage());
