 ...
 * Time: 15:04:03
 */
public class PolicyHistoryResponse extends CMHandlerBase {
    ...
    public void doProcess(CMMessage msg, Map holder) throws CMFault {
        ...
        try {
            ...
            JAXBContext context = JAXBContext.newInstance(PolicyHistoryResp.class);
            Marshaller marshaller = context.createMarshaller();
            marshaller.setProperty(Marshaller.JAXB_ENCODING, "UTF-8");
            marshaller.setProperty(Marshaller.JAXB_FORMATTED_OUTPUT, Boolean.TRUE);

            PolicyHistoryResp policyType = new PolicyHistoryResp();
       ...
       if (actionCode.equals(ActionCode.APPROVED)) {
            policyType.setPolicies((PolicyList) msg.getAttribute(Fields.POLICY_LIST));
        }
            marshaller.marshal(policyType, responseTypeXml);

            msg.setAttribute(CMMessage.RESPONSE, responseTypeXml.toString());
        } catch (Exception e) {
            log.error("ERROR :::Inside PolicyHistoryResponse.doProcess(): " + e.getMessage());
