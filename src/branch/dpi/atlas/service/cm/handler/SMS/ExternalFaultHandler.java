...
public class ExternalFaultHandler extends CMHandlerBase implements Configurable {
    ...
    public void doProcess(CMMessage msg, Map holder)
    {
        ...
        try {
            ...
            smsMessage.setActionCode(actionCode);
            int msgId = 1;
            ...
            msg.setAttribute(CMMessage.RESPONSE, smsMessage.createResponseHeader());
            ServiceLogger brLogger=new ServiceLogger();
            brLogger.process(msg, holder);
            connector.sendAsyncText(msg);

        } catch (Exception e) {
            log.error("Error ::: ExternalFaultHandler  for SMS >>> catching exception 1 ::: " + msg.getAttributeAsString(Fields.MESSAGE_ID) + " -- " + e.getMessage());
            log.debug("msg = " + msg);

            try {
                if (!msg.hasAttribute(CMMessage.RESPONSE)) {
                    log.error("Error ::: ExternalFaultHandler for SMS>>> catching exception 1 ::: There is no response, so it's set ");
                    smsMessage= (SMSMessage) msg.getAttribute(CMMessage.COMMAND_OBJ);
                    smsMessage.setActionCode(ActionCode.CHANNEL_CAN_NOT_CREATE_RESPONSE);
                    msg.setAttribute(CMMessage.RESPONSE, smsMessage.createResponseHeader());
                }
                connector.sendAsyncText(msg);
            } catch (Exception e1) {
                log.error("Error ::: ExternalFaultHandler for SMS>>> catching exception 2 ::: Error in sending message :: " + e.getMessage());
            }
