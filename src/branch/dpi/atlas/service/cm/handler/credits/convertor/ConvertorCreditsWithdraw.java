 ...
 * Time: 03:38 PM
 */
public class ConvertorCreditsWithdraw extends CreditsBaseConvertor implements CreditsToIMFFormater {
    @Override
    public CMCommand format(CreditsMessage creditsMessage) {
        ...
        command.addParam(Fields.REQUEST_TYPE, creditsMessage.getRequestType());  //WITHDRAW_TYPE   0:with min balance   1:without min balance
        command.setCommandName(TJCommand.CMD_CREDITS_WITHDRAW);
        command.addParam(Constants.CREDIT_DEBIT, Constants.CREDIT_VALUE);
        return command;
    }
