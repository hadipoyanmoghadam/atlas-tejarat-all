 ...
 * Time: 1:59 PM
 */
public class ConvertorFTRSafeBox extends AmxBaseConvertor implements AmxToIMFFormater {
    ...
    public CMCommand format(AmxMessage amxMessage) {
        ...
        if (amxMessage.getTerminalId().equalsIgnoreCase(Constants.TERMINAL_ID_SAFE_BOX))
            command.setCommandName(TJCommand.CMD_FUND_TRANSFER_REVERSE_SAFE_BOX);
        else
            command.setCommandName(TJCommand.CMD_FUND_TRANSFER_REVERSE_MARHOONAT_INSURANCE);
        ...
        command.addParam(Fields.BRANCH_DOC_NO, amxMessage.getDocumentNo());
        ...
        command.addParam(Fields.DATE, amxMessage.getRequestDate());
        command.addParam(Fields.TIME, amxMessage.getRequestTime());
