...
public class CheckTransaction4FollowUp extends TJServiceHandler implements Configurable {
    ...
    public void doProcess(CMMessage msg, Map holder) throws CMFault {
        ...
        if (!branchMsg.getPin().equalsIgnoreCase(Constants.PIN_FOLLOWUP))
            return;
        ...
        try {
            ...
            List<String> branchLogSet = ChannelFacadeNew.findBranchLogs(origMessageSequence,serviceType);
            if (branchLogSet.size() < 1)
                throw new NotFoundException(ActionCode.FLW_HAS_NO_ORIGINAL);
            if (serviceType.equalsIgnoreCase(Constants.BRANCH_SERVICE)) {
                String pin = branchLogSet.get(2);
                if (pin != null && (pin.equalsIgnoreCase(Constants.PIN_WITHDRAW_WAGE_RESPONSE) ||
                        pin.equalsIgnoreCase(Constants.PIN_REVERSE_WITHDRAW_WAGE_RESPONSE)))
                    msg.setAttribute(CMMessage.SERVICE_TYPE, Constants.BRANCH_WAGE_SERVICE);
            }
