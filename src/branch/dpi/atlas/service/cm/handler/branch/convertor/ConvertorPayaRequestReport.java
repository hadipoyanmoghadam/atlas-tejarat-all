 ...
 * Time: 04:35 PM
 */
public class ConvertorPayaRequestReport extends BranchBaseConvertor implements BranchToIMFFormater {
    ...
    public String createResponse(CMMessage msg, Map holder) throws CMFault {
        try {
            ...
            responseStr.append(branchMsg.createResponseHeader())
                    .append(ISOUtil.padleft(String.valueOf(requestList.size()), branchMsg.COUNT, '0'));

            for (int i = 0; i < requestList.size(); i++) {

                branchCode=requestList.get(i).getBranchSend().trim();
                responseStr.append(ISOUtil.padleft(branchCode, branchMsg.PAYA_BRANCH_CODE, ' '))
                ...
                .append(ISOUtil.padleft(requestList.get(i).getSerial(), branchMsg.SERIAL, ' '))
