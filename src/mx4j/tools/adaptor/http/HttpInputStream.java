...
//            variableHolder = new String(variableHolder.getBytes(), "UTF-8");
//            variableHolder = new String(variableHolder.getBytes("Cp037"), "UTF-8");
            ...
            while (parser.hasMoreTokens()) {
                ...
                if (equalIndex > 0) {
                    ...
                    if (command.indexOf("%7A") != -1)
                    {
                        variableValue = URLDecoder.decode(command.substring(equalIndex + 1, command.length()), "Cp037");
                    }
                    else
                    {
                        variableValue = URLDecoder.decode(command.substring(equalIndex + 1, command.length()),"latin1");
                    }
