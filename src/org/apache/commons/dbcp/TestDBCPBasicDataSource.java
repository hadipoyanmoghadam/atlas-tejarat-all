 ...
 */
public class TestDBCPBasicDataSource implements DataSource {
    ...
    private Vector idleDBConnection = new Vector();
    private Vector workingDBConnection = new Vector();
    ...
    public Connection getConnection(String username, String password)
            throws SQLException {
//        return createConnectionPool().getConnection(username, password);
        return getDBConnection().getConnection();
    }
    ...
    private PooledDBConnection getAndRemoveFirstIdlDBConnection() {
        PooledDBConnection pooledDBConnection = null;
        try {
            pooledDBConnection = (PooledDBConnection) idleDBConnection.firstElement();
            idleDBConnection.remove(pooledDBConnection);
        } catch (Exception e) {
//            e.toString();  //To change body of catch statement use File | Settings | File Templates.
//            if(log.isInfoEnabled())         log.info("MQ Thread Pool increased its size.");
//            pooledMQConnection = new MQSession(connection, send_queueName, receive_queueName, "" + (size++));
            pooledDBConnection = new PooledDBConnection(getDriverClassName(), getUrl(), getUsername(), getPassword());
        }
        return pooledDBConnection;
    }


    private void addWorkingDBConnection(PooledDBConnection connection) {
        workingDBConnection.add(connection);
    }

    private void removeWorkingDBConnection(PooledDBConnection connection) {
        workingDBConnection.remove(connection);
    }


    private void addIdleDBConnection(PooledDBConnection connection) {
        idleDBConnection.add(connection);
    }


    public synchronized PooledDBConnection getDBConnection() {
        PooledDBConnection pooledDBConnection = getAndRemoveFirstIdlDBConnection();
        addWorkingDBConnection(pooledDBConnection);
        return pooledDBConnection;
    }

    public synchronized void closeDBConnection(PooledDBConnection connection) {
        removeWorkingDBConnection(connection);
        addIdleDBConnection(connection);
    }


    public int getIdleConnectionCount() {
        return idleDBConnection.size();
    }

    public int getWorkingConnectionCount() {
        return workingDBConnection.size();
    }
    ...
    protected void createConnectionPool()
        ...
        for (int i = 0; i < getInitialSize(); i++) {
            addIdleDBConnection(new PooledDBConnection(getDriverClassName(), getUrl(), getUsername(), getPassword()));
        }
    }

/*
    protected synchronized DataSource createConnectionPool()
            throws SQLException {
        if (dataSource != null)
            return dataSource;
        ...
        System.out.println("connectionPool.getMaxActive() = " + connectionPool.getMaxActive());
        System.out.println("connectionPool.getMaxWait() = " + connectionPool.getMaxWait());
        System.out.println("connectionPool.getTestWhileIdle() = " + connectionPool.getTestWhileIdle());
        System.out.println("connectionPool.getMinEvictableIdleTimeMillis() = " + connectionPool.getMinEvictableIdleTimeMillis());
        System.out.println("connectionPool.getMinIdle() = " + connectionPool.getMinIdle());
        System.out.println("connectionPool.getNumActive() = " + connectionPool.getNumActive());
        System.out.println("connectionPool.getNumIdle() = " + connectionPool.getNumIdle());
        System.out.println("connectionPool.getNumTestsPerEvictionRun() = " + connectionPool.getNumTestsPerEvictionRun());
        System.out.println("connectionPool.getTestOnBorrow() = " + connectionPool.getTestOnBorrow());
        System.out.println("connectionPool.getTestOnReturn() = " + connectionPool.getTestOnReturn());
        System.out.println("connectionPool.getTestWhileIdle() = " + connectionPool.getTestWhileIdle());
        System.out.println("connectionPool.getTimeBetweenEvictionRunsMillis() = " + connectionPool.getTimeBetweenEvictionRunsMillis());
        System.out.println("connectionPool.getWhenExhaustedAction() = " + connectionPool.getWhenExhaustedAction());
        return dataSource;
    }
*/
